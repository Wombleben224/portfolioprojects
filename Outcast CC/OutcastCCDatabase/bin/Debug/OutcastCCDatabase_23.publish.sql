...
GO
/*
The column [dbo].[Members].[Username] on table [dbo].[Members] must be added, but the column has no default value and does not allow NULL values. If the table contains data, the ALTER script will not work. To avoid this issue you must either: add a default value to the column, mark it as allowing NULL values, or enable the generation of smart-defaults as a deployment option.
*/
...
GO
ALTER TABLE [dbo].[AspNetUsers] DROP CONSTRAINT [FK__AspNetUse__Membe__6B24EA82];
...
IF EXISTS (SELECT TOP 1 1 
           FROM   [dbo].[Members])
    BEGIN
        INSERT INTO [dbo].[tmp_ms_xx_Members] ([memberId], [VehicleYear], [VehicleMake], [VehicleModel], [Name], [Bio], [ProfileImageType], [ProfileImageName])
        SELECT   [memberId],
                 [VehicleYear],
                 [VehicleMake],
                 [VehicleModel],
...
ALTER TABLE [dbo].[AspNetUsers] WITH NOCHECK
    ADD FOREIGN KEY ([MemberId]) REFERENCES [dbo].[Members] ([memberId]);
...
DECLARE tableconstraintnames CURSOR LOCAL FORWARD_ONLY
    FOR SELECT SCHEMA_NAME([schema_id]),
               ...
               0
        FROM   [sys].[objects]
        WHERE  [parent_object_id] IN (OBJECT_ID(N'dbo.AspNetUsers'))
               AND [type] IN (N'F', N'C')
